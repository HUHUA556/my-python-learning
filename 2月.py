...
#总结
'''
若a=[1,2,3,4,5]  b=[1,2,1]
   追加单个：a.append(想要追加元素)
   追加多个：a.extend(想要追加的容器，如b)
   删除元素：a.remove(想要删除的元素)
统计某个元素：a.count(想要统计的元素)
   查询下标：a.index(想要查询的元素)
   清除容器：a.clear()
   
   插入元素：a.insert(插在哪里的下标，所需插入元素)
   取出元素：a.pop(想要取出元素的下标)  
   删除元素：del a[下标]
'''
